...
class UserProduct
{
    ...
    {
        $pdo = new PDO;
        $stmt = $pdo->getPdo()->prepare("SELECT products.name AS product_name, products.image, products.description, products.price, users.name AS user_name, user_products.amount 
        FROM user_products 
    ...
    {
        $pdo = new PDO;
        $stmt = $pdo->getPdo()->prepare("SELECT * FROM user_products WHERE user_id = :user_id AND product_id = :product_id");
    ...
    {
        $pdo = new PDO;
        $stmt = $pdo->getPdo()->prepare("SELECT amount FROM user_products WHERE user_id = :user_id AND product_id = :product_id");
        $stmt->execute(['user_id' => $user_id, 'product_id' => $product_id]);
        $result = $stmt->fetch();
